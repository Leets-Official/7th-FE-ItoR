...
function ErrorIcon() {
  return (
    <svg className="toast__icon" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      <path
...
export function Toast({ className, message, variant = 'error' }: ToastProps) {
  ...
  return (
    <div
      ...
      className={cn('toast', isError ? 'toast--error' : 'toast--success', className)}
    >
      {isError ? <ErrorIcon /> : <DoneIcon />}
      <p className="toast__message">{resolvedMessage}</p>
    </div>
  )
}
